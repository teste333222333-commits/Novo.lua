...
local function CreateESP(player)
    ...
    line.Visible = false
    line.Color = typeof(Settings.SkeletonColor) == "Color3" and Settings.SkeletonColor or Color3.fromRGB(255, 255, 255)
    line.Thickness = tonumber(Settings.SkeletonThickness) or 1
    line.Transparency = tonumber(Settings.SkeletonTransparency) or 1
end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            if not Drawings.ESP[player] then
                CreateESP(player, Settings.ESPColor or Color3.fromRGB(255, 255, 255), Settings.Thickness or 1)
            end
            UpdateESP(player)
        end
    end
